...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        localStorage.setItem('user_name', data.name || name || 'User');
        navigate('/chat');
      } else {
  ...
  const handleSocialAuth = (method: 'google' | 'phone') => {
    if (method === 'phone') {
      ...
      >
        <div className="bg-slate-900/40 border border-slate-800 p-8 rounded-3xl backdrop-blur-xl shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              disabled={isLoading}
              className="w-full group relative flex items-center justify-center gap-2 py-3 bg-blue-600 hover:bg-blue-500 rounded-2xl font-semibold transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-blue-600/25 disabled:opacity-70 disabled:hover:scale-100"
            >
              {isLoading ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button 
              type="button"
              onClick={() => loginMethod === 'phone' ? setLoginMethod('email') : handleSocialAuth('phone')}
              className="flex items-center justify-center gap-2 py-3 bg-slate-950/50 border border-slate-800 rounded-2xl hover:bg-slate-800 transition-colors"
            >
              {loginMethod === 'phone' ? (
                <>
